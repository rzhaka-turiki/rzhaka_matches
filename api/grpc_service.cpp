...
MatchServiceImpl::MatchServiceImpl(Database& db, const std::string& api_key) : db_(db), api_key_(api_key) {}
...
grpc::Status MatchServiceImpl::AddToken(grpc::ServerContext* ctx, const matchapi::AddTokenRequest* req,
    ...
    try {
        ...
        auto res = txn.exec_params(
            ...
            req->activation(), req->expiration(), req->stats_token(), req->admin_token(), req->player_token());
...
grpc::Status MatchServiceImpl::ListTokens(grpc::ServerContext* ctx, const matchapi::ListTokensRequest* req,
                                          matchapi::ListTokensResponse* resp) {
    if (!check_auth(ctx)) return grpc::Status(grpc::StatusCode::UNAUTHENTICATED, "Invalid API key");
    try {
        std::string sql =
            "SELECT id, activation, expiration, "
            "(NOW() BETWEEN activation AND expiration) as active FROM tokens";
        if (req->only_active()) sql += " WHERE NOW() BETWEEN activation AND expiration";
...
>>>>>>> a2ca35c (string fix)
        ...
        pqxx::work txn(db_.connection());
        auto res = txn.exec_params(sql, params);
        for (const auto& row : res) {
...
grpc::Status MatchServiceImpl::GetMatchStats(grpc::ServerContext* ctx, const matchapi::MatchStatsRequest* req,
    ...
    try {
        ...
        std::string cond;
        if (!req->start_date().empty()) cond += " AND match_start >= '" + req->start_date() + "'::timestamptz";
        if (!req->end_date().empty()) cond += " AND match_start <= '" + req->end_date() + "'::timestamptz";
        if (!req->map_name().empty()) cond += " AND map_name = '" + req->map_name() + "'";
        if (!req->player_hash().empty())
        ...
        sql += cond +
               ")) as total_players "
               "FROM matches m "
               "JOIN match_players ps ON ps.match_id = m.id WHERE 1=1" +
               cond;
...
grpc::Status MatchServiceImpl::GetMatchDetail(grpc::ServerContext* ctx, const matchapi::MatchDetailRequest* req,
    ...
    try {
        ...
        auto match_res = txn.exec_params("SELECT id, mid, map_name, aim_assist, match_start FROM matches WHERE id=$1",
                                         req->match_id());
        if (match_res.empty()) return grpc::Status(grpc::StatusCode::NOT_FOUND, "Match not found");
        auto& m = match_res[0];
        ...
        auto players = txn.exec_params(
            "SELECT player_name, character_name, kills, assists, knockdowns, "
            "damage_dealt, survival_time, team_name, team_placement, nid_hash "
            "FROM match_players WHERE match_id=$1 ORDER BY id",
            req->match_id());
...
grpc::Status MatchServiceImpl::GetPlayerStats(grpc::ServerContext* ctx, const matchapi::PlayerStatsRequest* req,
